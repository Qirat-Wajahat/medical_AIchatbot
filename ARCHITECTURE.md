...
└─────────────────┬───────────────────────────────────────────┘
                  ...
                  │                                          │
        ┌─────────▼──────────┐                  ┌───────────▼────────┐
        │  NLP Preprocessing │                  │  Disease Predictor │
        │  (preprocessing.py)│                  │ (disease_predictor)│
        │                    │                  │                    │
        │  - Text Cleaning   │                  │  - ML Model (NB)   │
        │  - Tokenization    │                  │  - TF-IDF          │
        │  - Lemmatization   │                  │  - Rule Matching   │
        │  - Stopword Remove │                  │  - Info Retrieval  │
        └────────────────────┘                  └──────────┬─────────┘
                                                           │
                                                           │
                                           ┌───────────────▼────────┐
...
- Model: ML models and data handlers

**Caching Strategy**:
- `@st.cache_resource` for model loading to prevent retraining on each interaction
...
- **Source**: medicines.json (10 diseases with symptoms)
...
- XSS: Handled by Streamlit's rendering
